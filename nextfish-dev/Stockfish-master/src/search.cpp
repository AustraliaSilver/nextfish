...
void Search::Worker::iterative_deepening() {
    ...
    const int aawRecenterWeight = int(options["AAW Recenter Weight"]);
    ...
    const int aawBlackEvalDisable = int(options["AAW Black Eval Disable"]);
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            int deltaLo = 5 + int(threadIdx % 8) + std::abs(rootMoves[pvIdx].meanSquaredScore) / 9000;
            int deltaHi = deltaLo;
            delta       = deltaLo;
            ...
            const bool blackEvalGate =
              !(us == BLACK && aawBlackConservative && int(avg) <= -aawBlackEvalDisable);
            const bool useAAW = aawEnabled && rootDepth >= 6 && blackDriftGate && blackEvalGate;
            ...
            if (useAAW)
            {
                ...
                {
                    const Move m = rootMoves[size_t(i)].pv[0];
                    if (m != Move::none()
                        && (rootPos.capture_stage(m) || rootPos.gives_check(m) || m.type_of() == PROMOTION))
                        ++tacticalCount;
                }
                deltaLo += tacticalCount * 2;
                deltaHi += tacticalCount * 2;

                if (mainThread)
                {
                    const TimePoint opt = mainThread->tm.optimum();
                    if (opt > 0)
                    {
                ...
                if (trend > 8)
                {
                    ...
                    deltaLo = std::max(aawMinDelta, deltaLo - aawTrendAsymmetry / 3);
                }
                else if (trend < -8)
                {
                    deltaLo += aawTrendAsymmetry;
                    deltaHi = std::max(aawMinDelta, deltaHi - aawTrendAsymmetry / 3);
                }
            ...
            while (true)
            {
                ...
                if (bestValue <= alpha)
                {
                    ...
                    if (useAAW)
                    {
                        ...
                        const int miss = int(alpha - bestValue);
                            ...
                            && !(us == BLACK && aawBlackConservative))
                        {
                            ...
                            beta = std::min(symmetricBeta, directionalBeta);
                        }
                        else
                            beta = symmetricBeta;
                        if (beta <= alpha)
                            beta = std::min(alpha + std::max(8, aawOppositeMargin), VALUE_INFINITE);
                ...
                else if (bestValue >= beta)
                {
                    ...
                    if (useAAW)
                    {
                        ...
                        const int miss = int(bestValue - beta);
                        int recenterBase = aawRecenterWeight;
                        if (avg < -20)
                            recenterBase = std::max(8, recenterBase - aawDefenseBoost / 2);
                        int recenterCap = pvStable ? 70 : 50;
                            ...
                            && !(us == BLACK && aawBlackConservative))
                        {
                            ...
                            alpha = std::max(symmetricAlpha, directionalAlpha);
                        }
                        else
                            alpha = symmetricAlpha;
                        if (beta <= alpha)
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (us == BLACK)
        {
            ...
            // If losing (eval < -40), reduce pruning even more to secure draws.
            if (ss->staticEval < -40)
                r -= 768;
