...
void Search::Worker::ensure_network_replicated() { (void) (networks[numaAccessToken]); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            // DEE-aware aspiration window: widen in tactical positions
            DEE::DEE_Result rootDee = {};
            if (options["Use DEE/HARENN"])
                rootDee = DEE::Evaluator::evaluate(rootPos);

            if (rootDee.is_tactical || std::abs(rootDee.threat_value) > 100)
                delta += 12;  // Wider window in tactical positions

            alpha = std::max(avg - delta, -VALUE_INFINITE);
            beta  = std::min(avg + delta, VALUE_INFINITE);

            // Dynamic Optimism & Draw Aversion
            // We increase optimism slightly and add a bias to avoid draws in complex positions
            int drawAversion = (rootMoves[pvIdx].pv.size() > 4) ? 15 : 0;
            optimism[us]     = (156 * avg / (std::abs(avg) + 98)) + drawAversion;
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int(2100 / 128.0 * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    // TT prefetch
    _mm_prefetch((const char*) tt.first_entry(pos.key()), _MM_HINT_T0);
    ...
    // DEE/HARENN at depth 12+ (only if enabled, only PV nodes)
    if (depth >= 12 && PvNode && options["Use DEE/HARENN"])
    {
    ...
    Value                 nmpMargin;
    const PieceToHistory* contHist[6];
        ...
        return qsearch<NonPV>(pos, ss, alpha, beta);
    {
        auto futility_margin = [&](Depth d) {
            Value futilityMult = 70 - 20 * !ss->ttHit;
            // DEE: widen margin in tactical positions to prune less
            if (options["Use DEE/HARENN"])
            {
                if (ss->dee.threat_value > 60)
                    futilityMult += 15;
            }
    ...
    // NMP margin: adjust based on HARENN tactical complexity
    nmpMargin = 320;
    if (options["Use DEE/HARENN"])
    {
        if (ss->harenn.tau > 0.50f)
            nmpMargin += 30;  // Harder to null-move in tactical positions
        if (ss->harenn.tau < 0.10f)
            nmpMargin -= 20;  // Easier to null-move in quiet positions
    }
    if (cutNode && ss->staticEval >= beta - 18 * depth + nmpMargin && !excludedMove
...
moves_loop:
    ...
    contHist[0] = (ss - 1)->continuationHistory;
    contHist[1] = (ss - 2)->continuationHistory;
    contHist[2] = (ss - 3)->continuationHistory;
    contHist[3] = (ss - 4)->continuationHistory;
    contHist[4] = (ss - 5)->continuationHistory;
    contHist[5] = (ss - 6)->continuationHistory;
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        // DEE/HARENN only affect LMR adjustments, not pruning decisions
        ...
        // DEE-aware singular extension
        int singularAdj = 0;
        if (options["Use DEE/HARENN"])
        {
            if (ss->dee.threat_value > 160)
                singularAdj = -8;  // Be more aggressive in tactical positions
            else if (ss->dee.threat_value < -160)
                singularAdj = 5;  // Be less aggressive when ahead
        }
        ...
        {
            Value singularBeta =
              ttData.value - (53 + singularAdj + 75 * (ss->ttPv && !PvNode)) * depth / 60;
        ...
        if (allNode)
            r += r / (depth + 1);
        // Tactical-Aware LMR using HARENN and DEE
        if (depth >= 2 && moveCount > 1)
        {
            int adj = 0;
            if (options["Use DEE/HARENN"])
            {
                // High tactical complexity (HARENN) -> Reduce LMR to be safer
                if (ss->harenn.tau > 0.45f)
                    adj -= 1;
                if (ss->harenn.tau > 0.75f)
                    adj -= 1;

                // Significant threats detected (DEE) -> Reduce LMR
                if (ss->dee.threat_value > 100)
                    adj -= 1;

                // Quiet positions (Low HARENN tau) -> Increase LMR to search deeper elsewhere
                if (ss->harenn.tau < 0.10f && depth > 6)
                    adj += 1;
            }

            int   reducedR = std::max(0, r + adj * 1024);
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    posKey = pos.key();
    // TT prefetch
    _mm_prefetch((const char*) tt.first_entry(posKey), _MM_HINT_T0);
    ...
    if (!is_decisive(bestValue) && bestValue > beta)
        bestValue = (bestValue + beta) / 2;
    if (!ss->inCheck && !moveCount && !pos.non_pawn_material(pos.side_to_move())
        && type_of(pos.captured_piece()) >= ROOK)
    {
        if (!((pos.side_to_move() == WHITE ? shift<NORTH>(pos.pieces(WHITE, PAWN))
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    int reductionScale = int(2747 / 128.0 * std::log(d)) * int(2747 / 128.0 * std::log(mn));
...
void update_all_stats(const Position& pos,
    ...
    int                    bonus =
      std::min(125 * depth - 90, 1580) + 380 * (bestMove == ttMove) + (ss - 1)->statScore / 28;

    // DEE-aware history gravity: amplify bonus if we find a good move under threat
    if (ss->dee.threat_value > 80)
        bonus += bonus * std::min(int(ss->dee.threat_value), 240) / 256;

    int malus = std::min(900 * depth - 220, 2600) - 20 * moveCount;
