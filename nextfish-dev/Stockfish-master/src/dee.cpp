...
Value Evaluator::evaluate_king_safety(const Position& pos, Color c, Bitboard enemyAttacks) {
    ...
    if (enemyAttacks & square_bb(ksq))
    {
        ...
        penalty -= popcount(kingAttackers) * 20;
    }

    Bitboard pinned = pos.blockers_for_king(c) & pos.pieces(c);
    if (pinned)
        penalty -= popcount(pinned) * 8;
...
DEE_Result Evaluator::evaluate(const Position& pos) {
    ...
    Bitboard ourValuable = pos.pieces(us) & ~pos.pieces(PAWN) & ~pos.pieces(KING);
    Bitboard hanging     = ourValuable & theirAttacks & ~ourAttacks;
    ...
    while (hanging)
    {
        ...
        threat -= PieceValue[pos.piece_on(s)] / 3;
    }

    Bitboard theirValuable = pos.pieces(them) & ~pos.pieces(PAWN) & ~pos.pieces(KING);
    Bitboard theirHanging  = theirValuable & ourAttacks & ~theirAttacks;
    while (theirHanging)
    {
    ...
    result.is_tactical = popcount(contested) >= 3;
