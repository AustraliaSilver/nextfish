...
MCTSNode::MCTSNode(Move m, MCTSNode* par, double movePrior) 
    : move(m), parent(par), priorScore(movePrior), isExpanded(false), isTerminal(false) {
    visits.store(0);
...
MCTSNode* MCTSNode::bestChild(double explorationConstant) const {
    ...
    const int parentVisits = std::max(1, visits.load());
    const double dynamicExploration =
      explorationConstant * (0.65 + 0.85 / (1.0 + parentVisits / 64.0));

    for (const auto& child : children) {
        const int nodeVisits = child->visits.load();
        const double parentPerspectiveQ =
          nodeVisits > 0 ? 1.0 - (child->totalScore.load() / nodeVisits) : child->priorScore;

        const double exploration = nodeVisits > 0
                                 ? dynamicExploration
                                     * std::sqrt(std::log(parentVisits + 1.0) / nodeVisits)
                                 : dynamicExploration * std::sqrt(std::log(parentVisits + 1.0));
        const double priorBonus =
          child->priorScore * std::max(0.03, 0.22 - nodeVisits * 0.007);
        const double score = parentPerspectiveQ + exploration + priorBonus;
...
bool MCTSTree::isNoisyMove(const Position& pos, Move move) const {
    return pos.capture_stage(move) || pos.gives_check(move) || move.type_of() == PROMOTION;
}

Move MCTSTree::selectDiversifiedMove(const std::vector<MovePriorCandidate>& candidates, int ply) {
    if (candidates.empty())
        return Move::none();
    ...
    const double greedyProbability = ply <= 1 ? 0.90 : 0.95;
    if (unit(rng) < greedyProbability)
        return candidates.front().move;

    const int poolSize = std::min<int>(3, int(candidates.size()));
    const double temperature = ply <= 1 ? 0.10 : 0.08;
    const double basePrior = candidates.front().prior;
    std::vector<double> weights(size_t(poolSize), 1.0);

    for (int i = 0; i < poolSize; ++i)
    {
        const double gap = std::max(0.0, basePrior - candidates[size_t(i)].prior);
        double weight = std::exp(-gap / temperature);
        if (i == poolSize - 1)
            weight += 0.04;
        weights[size_t(i)] = std::max(0.001, weight);
    }

    std::discrete_distribution<int> dist(weights.begin(), weights.end());
    return candidates[size_t(dist(rng))].move;
}
...
MCTSNode* MCTSTree::expandWithPrior(MCTSNode* node, Position& rootPos, MCTSStack& stack) {
    ...
    std::vector<MovePriorCandidate> candidates;
    candidates.reserve(size_t(legals.size()));
    ...
    for (const auto& m : legals)
    {
        ...
        if (!found)
        {
            double prior = calculateMovePrior(rootPos, m);
            if (isNoisyMove(rootPos, m))
                prior += 0.03;
            candidates.push_back({m, std::min(0.99, prior)});
        }
    }
    ...
    std::sort(candidates.begin(), candidates.end(),
              [](const MovePriorCandidate& a, const MovePriorCandidate& b) { return a.prior > b.prior; });

    const Move selectedMove =
      maxIterations < 96 ? candidates.front().move : selectDiversifiedMove(candidates, stack.depth);
    if (selectedMove == Move::none())
        return node;

    double selectedPrior = candidates.front().prior;
    for (const auto& c : candidates)
        if (c.move == selectedMove)
        {
            selectedPrior = c.prior;
            break;
        }

    MCTSNode* child = node->addChild(selectedMove, selectedPrior);

    // Keep child position on board for simulation phase.
    stack.push(rootPos, selectedMove);
...
double MCTSTree::simulateNNUE(MCTSNode* node, Position& rootPos, const Eval::NNUE::Networks& networks, MCTSStack& stack) {
    ...
    const Color rolloutSide = rootPos.side_to_move();
    if (maxIterations < 96 || maxSimDepth <= 1)
        return evaluateNNUE(rootPos, rolloutSide, networks);

    const double startScore = evaluateNNUE(rootPos, rolloutSide, networks);
    const int totalDepthLimit = maxSimDepth + quiescenceDepth;
    int rolloutPly = 0;

    while (rolloutPly < totalDepthLimit)
    {
        if (rootPos.is_draw(0))
            return 0.5;
        ...
        {
            if (rootPos.checkers())
                return rootPos.side_to_move() == rolloutSide ? 0.0 : 1.0;
            return 0.5;
        }
        ...
        const bool inQuiescenceWindow = rolloutPly >= maxSimDepth;
        if (inQuiescenceWindow && !rootPos.checkers() && !hasNoisy)
            break;

        const Move rolloutMove = selectDiversifiedMove(candidates, rolloutPly);
        if (rolloutMove == Move::none())
            break;

        stack.push(rootPos, rolloutMove);
        ++rolloutPly;
    }

    const double leafScore = evaluateNNUE(rootPos, rolloutSide, networks);
    const double leafBlend = std::clamp(0.45 + 0.03 * rolloutPly, 0.45, 0.65);
    return startScore * (1.0 - leafBlend) + leafScore * leafBlend;
}
...
Move MCTSTree::getBestMove(MCTSNode* root, double* outWinRate, int* outVisits, int* outRootVisits) const {
    ...
    for (const auto& c : root->children) {
        ...
        double wr = 1.0 - (c->totalScore.load() / v);
    ...
    if (best && outWinRate) *outWinRate = 1.0 - (best->totalScore.load() / best->visits.load());
...
void MCTSTree::collectRootStats(MCTSNode* root, std::vector<MCTSRootStat>* outRootStats) const {
    ...
    for (const auto& child : root->children) {
        ...
        stat.visits = visits;
        stat.winRate = 1.0 - (child->totalScore.load() / visits);
...
Move ShashinManager::runMCTSSearch(Position& pos, const Eval::NNUE::Networks& networks, int iterations, 
    ...
    const int requestedIterations = iterations > 0 ? iterations : config.mctsIterations;
    const int safeIterations = std::clamp(requestedIterations, 1, 10000);
    MCTSTree tree(safeIterations, config.mctsExploration, currentStyle);
