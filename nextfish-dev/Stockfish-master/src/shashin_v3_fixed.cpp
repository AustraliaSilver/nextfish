/*
  Nextfish - Shashin Theory Implementation v3 (Fixed)
  Highly Optimized MCTS with 300 iterations
...
double MCTSTree::calculateMovePrior(Position& pos, Move move) const {
    ...
    if (pos.capture_stage(move)) {
        ...
        if (captured != NO_PIECE) {
            // Use piece value as proxy for SEE
            int captureValue = PieceValue[type_of(captured)];
            int attackerValue = PieceValue[type_of(pos.moved_piece(move))];
            int seeValue = captureValue - attackerValue / 10; // Approximate
    ...
    while (depth < maxSimDepth) {
        ...
        if (depth >= 25 || inQuiescence) {
            ...
            for (const auto& m : legalMoves) {
                ...
                if (isCapture || isCheck) {
                    int score = isCapture ? PieceValue[type_of(rootPos.piece_on(m.to_sq()))] : 0;
                    if (isCheck) score += 50;
        ...
        for (const auto& m : legalMoves) {
            ...
            if (rootPos.capture_stage(m)) {
                ...
                if (captured != NO_PIECE) {
                    int captureValue = PieceValue[type_of(captured)];
                    int attackerValue = PieceValue[type_of(rootPos.moved_piece(m))];
                    int see = captureValue - attackerValue / 10;
...
Move ShashinManager::runMCTSSearch(Position& pos, int iterations) {
    ...
    // Use 300 iterations for better quality
