...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            delta     = 4 + threadIdx % 8 + std::abs(rootMoves[pvIdx].meanSquaredScore) / 9000;
            Value avg = rootMoves[pvIdx].averageScore;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Nextfish v25 (Omniscient Warden): Adaptive Aggression.
            // Balance midgame pressure with extreme endgame precision.
            int pieces = rootPos.count<ALL_PIECES>();
            int baseAgg = (us == WHITE) ? 12 : 10;
            if (std::abs(avg) > 250) baseAgg -= 4;
            
            // Nextfish Strategy: Consult the oracle
            // Logic được đóng gói trong Nextfish::Strategy
            auto advice = Nextfish::Strategy::consult(us, rootPos, ss, rootDepth, 0); 
            
            int sideAggression = baseAgg + (pieces * pieces) / 48;
            sideAggression += advice.optimismAdjustment;

            optimism[us]  = 142 * avg / (std::abs(avg) + 91) + sideAggression;
            ...
            while (true)
            {
                ...
                if (bestValue <= alpha)
                {
                    ...
                    // Nextfish v25: Trigger Warden Surge on re-search
                    ss->aspirationSurge = true; 
                }
        ...
        if (limits.use_time_management() && !threads.stop && !mainThread->stopOnPonderhit)
        {
            ...
            // Nextfish v63: Singularity Reborn - Dynamic Non-linear Logic
            int scoreDiff = 0;
            if (mainThread->bestPreviousScore != VALUE_INFINITE) {
                 scoreDiff = std::abs(bestValue - mainThread->bestPreviousScore);
            }

            mainThread->tm.update_pulse(int(totBestMoveChanges), scoreDiff);

            double totalTime = double(mainThread->tm.optimum());
            
            timeReduction = 1.0; 

            // Nextfish Strategy: Time Management
            totalTime *= Nextfish::Strategy::getTimeFactor(us);
  ...
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    // The depth condition is important for mate finding.
    {
        auto futility_margin = [&](Depth d) {
            ...
            // Nextfish v14: Adaptive Defensive Pruning
            // Prune less when behind to find defensive resources
            int evalDampening = (eval < -100) ? eval / 20 : 0;
            int depthDampening = (d > 20) ? (d - 20) * 2 : 0;

            return futilityMult * d
                 - (2474 * improving + 331 * opponentWorsening) * futilityMult / 1024  //
                 + std::abs(correctionValue) / 174665 + depthDampening + evalDampening;
        };
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        Depth r = reduction(improving, depth, moveCount, delta, pos, ss);
        ...
        {
            Value singularBeta  = ttData.value - (54 + 80 * (ss->ttPv && !PvNode)) * depth / 60;
        ...
        // Nextfish v17: Kawaii Shark LMR - Defensive Resilience & Tactical Shield
        if (us == BLACK && bestValue < 0) r -= 80; // Kawaii Shield: Search deeper when behind as Black
        else if (us == BLACK && !improving) r -= 40;
        
        if (bestValue > 150) r -= 150;

        // Reduce reduction further if moving a powerful piece (Queen/Rook) in enemy territory
        if (type_of(movedPiece) >= ROOK && relative_rank(us, move.to_sq()) >= RANK_5)
            r -= 32;
        ...
        // Nextfish v7: Added a small bonus for non-drawish outcomes in endgame.
        int endgameContempt = (pos.count<ALL_PIECES>() < 12 && value > -50 && value < 50) ? 1 : 0;
        int inc = (value + endgameContempt == bestValue && ss->ply + 2 >= rootDepth && (int(nodes) & 14) == 0
                   && !is_win(std::abs(value) + 1));

        if (value + inc + endgameContempt > bestValue)
        {
            bestValue = value;
    ...
    else if (!priorCapture && prevSq != SQ_NONE)
    {
                    ...
                    bonusScale += std::min(65 * depth, 489);
                    bonusScale += 210 * ((ss - 1)->moveCount > 8);
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta, const Position& pos, Stack* ss) const {
    int reductionScale = reductions[d] * reductions[mn];
    int r = reductionScale - delta * 608 / rootDelta + !i * reductionScale * 256 / 512 + 1182;
    
    // Nextfish v25: Omniscient Warden Surge (Aspiration Window Recovery)
    if (ss->aspirationSurge) r -= 140; 

    // Nextfish v25: Dynamic Defensive Shield
    if (ss->staticEval < -80) {
        int deficit = -ss->staticEval;
        r -= std::clamp(deficit / 2 + 40, 0, 180);
        
        // Protect counter-play moves (high history score)
        if (ss->statScore > 0) r -= 80;
    }

    // Nextfish v25: Executioner's Precision (Winning Advantage)
    if (ss->staticEval > 150) r -= 180;

    // Nextfish v25: Sovereign King Shield
    Color us = pos.side_to_move();
    Square ksq = pos.square<KING>(us);
    if (pos.attackers_to(ksq, pos.pieces()) & pos.pieces(~us))
    {
        r -= 72; // Base King Safety Shield
        if (us == BLACK) r -= 48; // Enhanced Black Sovereign Shield
    }

    // Nextfish v25: Piece-Count Adaptive Scaling
    int allPieces = pos.count<ALL_PIECES>();
    int pieceBonus = r * allPieces / 150;
    r -= pieceBonus * (MAX_PLY - d) / (MAX_PLY - (allPieces < 12 ? 160 : 0));

    // Nextfish Strategy: Apply strategic scaling
    auto advice = Nextfish::Strategy::consult(us, pos, ss, d, mn);
    r = int(r * advice.reductionMultiplier) + advice.reductionAdjustment;
...
void update_all_stats(const Position& pos,
    ...
    int bonus =
      std::min(116 * depth - 81, 1515) + 500 * (bestMove == ttMove) + (ss - 1)->statScore / 32;
