...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            // Moderate contempt - balanced
            int contempt     = 12;
            int optimismBase = 142 * avg / (std::abs(avg) + 91);
            optimism[us]     = optimismBase + contempt;
            optimism[~us]    = -(optimismBase + contempt);
        ...
        if (limits.use_time_management() && !threads.stop && !mainThread->stopOnPonderhit)
        {
            ...
            if (elapsedTime > std::min(totalTime, double(mainThread->tm.maximum())))
            {
                ...
                threads.increaseDepth = mainThread->ponder || elapsedTime <= totalTime * 0.70;
        }
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!PvNode && eval < alpha - 440 - 260 * depth * depth)
    ...
    // The depth condition is important for mate finding.
    {
        auto futility_margin = [&](Depth d) {
            Value futilityMult = 72 - 20 * !ss->ttHit;

            return futilityMult * d
                 - (2300 * improving + 300 * opponentWorsening) * futilityMult / 1024  //
                 + std::abs(correctionValue) / 160000;
        };

        if (!ss->ttPv && depth < 13 && eval - futility_margin(depth) >= beta && eval >= beta
    ...
    while ((move = mp.next_move()) != Move::none())
    {
            ...
            && ttData.depth >= depth - 3 && !is_shuffling(move, ss, pos))
        {
            ...
            if (value < singularBeta)
            {
                ...
                int doubleMargin = -10 + 185 * PvNode - 180 * !ttCapture - corrValAdj
                                 - 897 * ttMoveHistory / 120000 - (ss->ply > rootDepth) * 40;
                int tripleMargin = 60 + 280 * PvNode - 230 * !ttCapture + 85 * ss->ttPv - corrValAdj
                                 - (ss->ply * 2 > rootDepth * 3) * 45;
        ...
        // Improvement: reduce less for quiet moves with good history
...
Value Search::Worker::evaluate(const Position& pos) {
    ...
    {
        DEE::DEE_Result deeRes = DEE::Evaluator::evaluate(pos);
        HARENN::EvalResult harennRes = HARENN::GuidanceProvider::query(pos);

        // Integrate DEE king safety and threat values
        int deeBonus = int(deeRes.king_safety_delta) + int(deeRes.threat_value) / 24;
        
        // HARENN tau (tactical tension) and resolution score
        // We use tau to scale the importance of tactical features
        float tensionFactor = 1.0f + 0.35f * harennRes.tau;
        int harennBonus = int(harennRes.eval * 0.05f * tensionFactor);

        int totalBonus = deeBonus + harennBonus;
        
        // Depth-dependent scaling: less influence at very shallow depths
        // to avoid tactical noise in early tree pruning.
        int d = std::max(0, (int)completedDepth);
        totalBonus = totalBonus * std::min(128, 64 + d * 4) / 128;

        // Cap the total bonus to prevent over-evaluation and instability
        totalBonus = std::clamp(totalBonus, -100, 100);
